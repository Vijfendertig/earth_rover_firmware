...
#include <earth_rover_firmware_msgs/ControlServo.h>
#include <earth_rover_firmware_msgs/ControlServoRaw.h>
#include <earth_rover_firmware_msgs/ServoConfiguration.h>
#include <earth_rover_firmware_msgs/ConfigureServo.h>
...
namespace earth_rover_firmware {
  ...
  class RosserialServoController {
    private:  // Data types and member variables.
      ...
      ros::Subscriber<ControlServo, RosserialServoController> control_subscriber_;
      ros::Subscriber<ControlServoRaw, RosserialServoController> control_raw_subscriber_;
      ros::Subscriber<ConfigureServo, RosserialServoController> configure_subscriber_;
      struct StoredConfiguration {
        uint8_t signature_front;
        ConfiguredServo::Configuration configuration;
        uint8_t signature_rear;
      };
      uint16_t configuration_base_address_;
  ...
  template<int ...pin_numbers>
  RosserialServoController<pin_numbers...>::RosserialServoController(ros::NodeHandle * node_handle,
      uint16_t configuration_base_address):
    servo_count_{sizeof...(pin_numbers)},
    pin_numbers_{pin_numbers...},
    servos_{pin_numbers...},
    node_handle_{node_handle},
    control_subscriber_{"/servo/control", &RosserialServoController::controlCallback, this},
    control_raw_subscriber_{"/servo/control_raw", &RosserialServoController<pin_numbers...>::controlRawCallback, this},
    configure_subscriber_{"/servo/configure", &RosserialServoController::configureCallback, this},
    configuration_base_address_{configuration_base_address}
  {
    ;
  }
  ...
  template<int ...pin_numbers>
  void RosserialServoController<pin_numbers...>::setup() {
  ...
  template<int ...pin_numbers>
  void RosserialServoController<pin_numbers...>::controlCallback(const ControlServo & message) {
  ...
  template<int ...pin_numbers>
  void RosserialServoController<pin_numbers...>::controlRawCallback(const ControlServoRaw & message) {
    if(message.servo_id >= 0 && message.servo_id < servo_count_) {
      digitalWrite(LED_BUILTIN, !digitalRead(LED_BUILTIN));
  ...
  template<int ...pin_numbers>
  void RosserialServoController<pin_numbers...>::configureCallback(const ConfigureServo & message) {
    if(message.servo_id >= 0 && message.servo_id < servo_count_) {
  ...
  template<int ...pin_numbers>
  bool RosserialServoController<pin_numbers...>::loadConfigurationFromEeprom(uint8_t servo_id,
      ConfiguredServo::Configuration & configuration) {
  ...
  template<int ...pin_numbers>
  void RosserialServoController<pin_numbers...>::saveConfigurationToEeprom(uint8_t servo_id,
